...
util::Status DriverHelper::Submit(const TestVector& test_vector, int batches) {
  ...
  Buffer::NamedMap output_with_guard_areas;
  ...
  if (!input_names.empty()) {
    ...
    for (const auto& input_name : input_names) {
      ...
      ASSIGN_OR_RETURN(const int input_size,
                       executable_ref->InputLayerPaddedSizeBytes(input_name));
      auto batch_buffer = driver_->MakeBuffer(input_size * batches);
      for (int i = 0; i < batches; ++i) {
        ...
        Buffer input_buffer = batch_buffer.Slice(i * input_size, input_size);
  ...
  if (!output_names.empty()) {
    ...
    for (const auto& output_name : output_names) {
      ...
      // Allocate buffer with guard area.
      size_t output_plus_guard_area_size =
          output_size + (guard_area_size_bytes_ * 2);
      auto batch_buffer =
          driver_->MakeBuffer(output_plus_guard_area_size * batches);
      for (int i = 0; i < batches; ++i) {
        ...
        if (guard_area_size_bytes_ == 0) {
          // No guard area.
          auto output_buffer = batch_buffer.Slice(i * output_size, output_size);
          ...
          // Allocate buffer with guard area.
          auto output_buffer_with_guard = batch_buffer.Slice(
              i * output_plus_guard_area_size, output_plus_guard_area_size);

          Buffer leading_guard_area(output_buffer_with_guard.ptr(),
                                    guard_area_size_bytes_);
          FillAreaWithKnownPattern(leading_guard_area, GuardPattern);

          // memcpy is going to work as well, but having a separate buffer and
          // call to fill up is slightly more flexible regarding size and
          // pattern.
          Buffer trailing_guard_area(output_buffer_with_guard.ptr() +
                                         guard_area_size_bytes_ + output_size,
                                     guard_area_size_bytes_);
          FillAreaWithKnownPattern(trailing_guard_area, GuardPattern);

          Buffer output_buffer =
              Buffer(output_buffer_with_guard.ptr() + guard_area_size_bytes_,
                     output_size);
          ...
          output[output_name].push_back(std::move(output_buffer));
          output_with_guard_areas[output_name].push_back(
              std::move(output_buffer_with_guard));
        }
      }
  ...
  return Submit(test_vector.name(), test_vector.executable_reference(),
                test_vector.output_file_name(), input, expected_output, output,
