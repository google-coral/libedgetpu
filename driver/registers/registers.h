...
class Registers : public driver_shared::Registers {
 public:
  ...
  Status Poll(uint64 offset, uint64 expected_value) {
  ...
  // version.
  virtual Status Poll(uint64 offset, uint64 expected_value, int64 timeout_us);

  // 32-bit version of above. Usually, it is same as running 64 bit version.
  Status Poll32(uint64 offset, uint32 expected_value) {
    return Poll32(offset, expected_value, kInfiniteTimeout);
  }
  virtual Status Poll32(uint64 offset, uint32 expected_value, int64 timeout_us);
 ...
 protected:
  ...
  Status SpinReadHelper(uint64 offset, IntType expected_value, int64 timeout_us,
                        const FuncType& read_func) {
    ...
    while (actual_value != expected_value) {
      if (timeout_us > 0) {
        ...
        if (end_time_us - start_time_us > timeout_us) {
          return DeadlineExceededError("Register poll timeout.");
        }
      }
