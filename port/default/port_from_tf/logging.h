...
#ifndef DARWINN_PORT_DEFAULT_PORT_FROM_TF_LOGGING_H_
#define DARWINN_PORT_DEFAULT_PORT_FROM_TF_LOGGING_H_

#if defined(DARWINN_PORT_ANDROID_SYSTEM)

#ifndef LOG_TAG
#define LOG_TAG "Darwinn"
#endif
...
// C++ 14 (current Android runtime libc++) does not have overloads for
// std::nullptr_t. Add this as a work around until Android runtime libc++
// supports C++ 17.
static inline std::ostream& operator<<(std::ostream& s, std::nullptr_t) {
  return s << "nullptr";
}
...
#define _LOG_INFO                                                \
  ::platforms::darwinn::internal::LogMessage(__FILE__, __LINE__, \
                                             platforms::darwinn::INFO)
#define _LOG_WARNING                                             \
  ::platforms::darwinn::internal::LogMessage(__FILE__, __LINE__, \
                                             platforms::darwinn::WARNING)
#define _LOG_ERROR                                               \
  ::platforms::darwinn::internal::LogMessage(__FILE__, __LINE__, \
                                             platforms::darwinn::ERROR)
#define _LOG_FATAL \
  ::platforms::darwinn::internal::LogMessageFatal(__FILE__, __LINE__)
...
#define CHECK(condition) \
  if (PREDICT_FALSE(!(condition))) LOG(FATAL) << "Check failed: " #condition " "
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                    \
  ...
  inline std::string* name##Impl(const T1& v1, const T2& v2,              \
    ...
    if (PREDICT_TRUE(v1 op v2))                                           \
      ...
      return ::platforms::darwinn::internal::MakeCheckOpString(v1, v2,    \
                                                               exprtext); \
  }                                                                       \
...
#define CHECK_OP_LOG(name, op, val1, val2)                                    \
  while (::platforms::darwinn::internal::CheckOpString _result =              \
             ::platforms::darwinn::internal::name##Impl(                      \
                 ::platforms::darwinn::internal::GetReferenceableValue(val1), \
                 ::platforms::darwinn::internal::GetReferenceableValue(val2), \
                 #val1 " " #op " " #val2))                                    \
  ::platforms::darwinn::internal::LogMessageFatal(__FILE__, __LINE__)         \
      << *(_result.str_)
...
// Following macros are shared between default and Android.
namespace platforms {
namespace darwinn {
namespace internal {

// There is no system VLOG in Android runtime. Provide one here.
#undef VLOG_IS_ON
extern int GetLoggingLevel();
extern void SetLoggingLevel(int new_logging_level);
#define VLOG_IS_ON(lvl) \
  (lvl <= ::platforms::darwinn::internal::GetLoggingLevel())

#ifndef VLOG
#define VLOG(lvl) \
  if (VLOG_IS_ON(lvl)) LOG(INFO)
#endif
...
#endif  // DARWINN_PORT_DEFAULT_PORT_FROM_TF_LOGGING_H_
